...
function shipMaxFlares(s, world) {
  return s.isPlayer ? Math.floor(FLARE_MAX * (world.playerFlareMult || 1)) : FLARE_MAX;
}
...
function shipTick(s, world, dt) {
  ...
  // --- Auto-Defend EVASION: no flares + incoming locked torps → run at warp 6
  // straight away from the torp centroid until the threat clears. Overrides
  // orbit, autopilot, and target-lock speed cap (survival comes first).
  if (s.isPlayer && world.autoDefendEnabled && s.flareCount === 0) {
    ...
    if (count > 0) {
      ...
      s.desiredSpeed = 6;
      if (s.orbiting) leaveOrbit(s);
      if (s.autoPilot) s.autoPilot = null;
      if (!s._evading) {
        s._evading = true;
        pushMessage(world, `AUTO-DEFEND: no flares — evasive maneuvers at warp 6 (${count} torpedo${count > 1 ? "es" : ""} inbound).`, "warn");
      }
    } else if (s._evading) {
      s._evading = false;
      pushMessage(world, "AUTO-DEFEND: threat clear — resuming course.", "you");
    }
  } else if (s._evading) {
    // Auto-Defend turned off mid-evasion, or we picked up flares again.
    s._evading = false;
  }
...
function autoFireAtTarget(s, world) {
  ...
  const def = shipDef(s);
  const d = Math.hypot(t.x - s.x, t.y - s.y);
  // Phasers: wider cone (±30°), close-range — unrestricted rate (cooldown + energy gate it)
...
function damagePlanetDefense(p, dmg, world) {
  if (!p || !p.defenseMaxHull || p.defenseHull <= 0) return;
  p.defenseHull = Math.max(0, p.defenseHull - dmg);
  p.flashUntil = world.now + 0.4;
  if (p.defenseHull === 0) {
    pushMessage(world, `${p.name}'s defenses destroyed.`, "win");
  }
}
