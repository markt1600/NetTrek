...
function shipTick(s, world, dt) {
  ...
  // --- Auto-target nearest enemy (player only) ---
  if (s.isPlayer && world.autoTargetEnabled) {
    const cur = s.targetLock ? world.ships.find(o => o.id === s.targetLock && o.alive) : null;
    const tooFar = cur && Math.hypot(cur.x - s.x, cur.y - s.y) > LOCK_BREAK_RANGE;
    if (!cur || tooFar) {
      const candidate = nearestEnemyForLock(s, world);
      if (candidate && candidate.id !== s.targetLock) {
        s.targetLock = candidate.id;
        s.selectedPlanet = null;
        pushMessage(world, `Auto-lock acquired: ${candidate.team}/${candidate.shipClass} ${candidate.name}.`, "you");
      }
    }
  }
  ...
  if (s.isPlayer && s.autoPilot) {
    ...
    if (!tgt) {
      ...
      if (ap.type === "planet") {
        ...
        // For ship targets, autopilot disengages once we're in combat distance.
        const dToShip = Math.hypot(tgt.x - s.x, tgt.y - s.y);
        if (dToShip < shipDef(s).phaserRange) {
          s.autoPilot = null;
...
function autoFireAtTarget(s, world) {
  ...
  if (inPhaserCone(s, t) && d <= def.phaserRange &&
      s.phaserCool === 0 && s.energy >= def.phaserEnergy) {
