...
function newWorld(playerTeam, playerClass, difficulty, playerShieldMult) {
...
function startGame() {
  ...
  // player gets 10× shield HP. Everything else is a normal numeric multiplier.
  let diff, shieldMult;
  if (diffRaw === "cadet") { diff = 1.0; shieldMult = 10; }
  else { diff = parseFloat(diffRaw); shieldMult = 1; }
  world = newWorld(team, ship, diff, shieldMult);
  attachInput(world);
