...
function updateHud(world) {
  ...
  // Shields — % shown when activated, status text reflects up/down/reboot
  const shieldPct = me.shield / shipMaxShield(me, world);
  setBar("bar-shield", shieldPct, "blue", "blue", "red");
  let shieldStat;
  if (me.shieldsUp) shieldStat = `${Math.round(shieldPct * 100)}% UP`;
  else if (world.now < me.shieldCollapsedUntil) shieldStat = `REBOOT ${(me.shieldCollapsedUntil - world.now).toFixed(1)}s`;
  else shieldStat = `${Math.round(shieldPct * 100)}% DOWN`;
  setText("hud-shield-pct", shieldStat);
