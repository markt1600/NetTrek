...
function attachInput(world) {
  ...
  document.addEventListener("keydown", e => {
    ...
    if (k === "c") {
      // 1) Already orbiting? Start capture if the orbited planet is non-friendly.
      if (me.orbiting) {
        const p = world.planets.find(p => p.id === me.orbiting);
        if (p && p.team === me.team) {
          pushMessage(world, `${p.name} is already yours.`, "warn");
          return;
        }
      ...
      // 2) Not orbiting — if a non-friendly planet is selected, autopilot there
      //    and auto-start the capture sequence on orbit lock.
      if (me.selectedPlanet) {
        const p = world.planets.find(x => x.id === me.selectedPlanet);
        if (p && p.team === me.team) {
