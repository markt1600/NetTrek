...
function cycleZoom(world, delta) {
  ...
  world.zoomLevel = lvl;
  pushMessage(world, `Tactical zoom: ${ZOOM_LEVELS[lvl]}× (${ZOOM_LEVELS[lvl]} screens).`);
...
function attachInput(world) {
  ...
  tac.addEventListener("mousedown", e => {
    ...
    if (e.button === 2) {
      ...
      if (r.queued) {
        ...
        else if (me.energy < shipDef(me).phaserEnergy) pushMessage(world, "Insufficient energy for phasers.", "alert");
  ...
  document.addEventListener("keydown", e => {
    ...
    if (k === "l") {
      world.autoTargetEnabled = !world.autoTargetEnabled;
      pushMessage(world, `Auto-Target Closest: ${world.autoTargetEnabled ? "ON" : "OFF"}.`, world.autoTargetEnabled ? "you" : "warn");
      const btn = document.getElementById("btn-autolock");
      if (btn) btn.classList.toggle("active", world.autoTargetEnabled);
      return;
    }
